...
def main():
    ...
    american_regional_terms, regional_subs = add_american()
    ...
    for region, terms in regions.items():
        combined_regional_terms[region] = set(terms).union(regionalities.get(region, [])).union(american_regional_terms.get(region, []))
